...
public class MultiThreadedDefaultProvenanceClient extends DefaultProvenanceClient {
  ...
  private <T, TP> List<Future<?>> getByProvider(
      final Map<String, TP> providers,
      final Map<FileProvenanceFilter, Set<String>> filters,
      ExecutorService executorService,
      Map<String, Map<String, ? extends T>> output,
      Function<TP, Collection<? extends T>> getProvenance,
      BiFunction<TP, Map<FileProvenanceFilter, Set<String>>, Collection<? extends T>>
          getProvenanceWithFilter,
  ...
  private List<Future<?>> getSampleProvenanceFutureByProvider(
      final Map<FileProvenanceFilter, Set<String>> filters,
      ExecutorService executorService,
      Map<String, Map<String, ? extends SampleProvenance>> output)
      throws InterruptedException {
    return getByProvider(
        sampleProvenanceProviders,
        filters,
        executorService,
        output,
        SampleProvenanceProvider::getSampleProvenance,
        SampleProvenanceProvider::getSampleProvenance,
        SampleProvenance::getProvenanceId);
  }

  private List<Future<?>> getLaneProvenanceFutureByProvider(
      final Map<FileProvenanceFilter, Set<String>> filters,
      ExecutorService executorService,
      Map<String, Map<String, ? extends LaneProvenance>> output)
      throws InterruptedException {
    return getByProvider(
        laneProvenanceProviders,
        filters,
        executorService,
        output,
        LaneProvenanceProvider::getLaneProvenance,
        LaneProvenanceProvider::getLaneProvenance,
        LaneProvenance::getProvenanceId);
  }

  private List<Future<?>> getAnalysisProvenanceFutureByProvider(
      final Map<FileProvenanceFilter, Set<String>> filters,
      ExecutorService executorService,
      Map<String, Collection<AnalysisProvenance>> output)
      throws InterruptedException {
    List<Future<?>> futures = new ArrayList<>();
      ...
      futures.add(
          executorService.submit(
              () -> {
                ...
                log.info(
                    "Provider = [{}] completed getAnalysisProvenance() in {}",
                    provider,
                    sw.toString());
  ...
  public Collection<FileProvenance> getFileProvenance(
    ...
    try {
      ...
      futures.addAll(
          getSampleProvenanceFutureByProvider(
              Collections.<FileProvenanceFilter, Set<String>>emptyMap(), es, spsByProvider));
      Map<String, Map<String, ? extends LaneProvenance>> lpsByProvider = new HashMap<>();
      futures.addAll(
          getLaneProvenanceFutureByProvider(
              Collections.<FileProvenanceFilter, Set<String>>emptyMap(), es, lpsByProvider));
      ...
      return applyFileProvenanceFilters(
          getFileProvenance(spsByProvider, lpsByProvider, apsByProvider), filters);
