...
public class DefaultProvenanceClient implements ExtendedProvenanceClient, AutoCloseable {
  ...
  public AnalysisProvenanceProvider registerAnalysisProvenanceProvider(
      String provider, AnalysisProvenanceProvider app) {
    return analysisProvenanceProviders.put(provider, app);
  }

  public LaneProvenanceProvider registerLaneProvenanceProvider(
      String provider, LaneProvenanceProvider lpp) {
    return laneProvenanceProviders.put(provider, lpp);
  }

  public SampleProvenanceProvider registerSampleProvenanceProvider(
      String provider, SampleProvenanceProvider spp) {
  ...
  public Map<String, Map<String, ? extends SampleProvenance>> getSampleProvenanceByProviderAndId(
    ...
    for (Entry<String, Collection<? extends SampleProvenance>> e :
        getSampleProvenanceByProvider(filters).entrySet()) {
      String provider = e.getKey();
      ...
      for (SampleProvenance sp : e.getValue()) {
        if (spsById.put(sp.getProvenanceId(), sp) != null) {
          throw new RuntimeException(
              "Duplicate sample provenance ID = ["
                  + sp.getProvenanceId()
                  + "] from provider = ["
                  + provider
                  + "]");
  ...
  public Map<String, Map<String, ? extends LaneProvenance>> getLaneProvenanceByProviderAndId(
    ...
    for (Entry<String, Collection<? extends LaneProvenance>> e :
        getLaneProvenanceByProvider(filters).entrySet()) {
      String provider = e.getKey();
  ...
  public Collection<? extends SampleProvenance> getSampleProvenance() {
    ...
    for (Map<String, ? extends SampleProvenance> e :
        getSampleProvenanceByProviderAndId(null).values()) {
      sps.addAll(e.values());
    }
    return sps;
  }
  ...
  public Collection<? extends SampleProvenance> getSampleProvenance(
    ...
    List<SampleProvenance> sps = new ArrayList<>();
    for (Map<String, ? extends SampleProvenance> e :
        getSampleProvenanceByProviderAndId(filters).values()) {
      sps.addAll(e.values());
    }
    return sps;
  }
  ...
  public Collection<? extends LaneProvenance> getLaneProvenance() {
    ...
    for (Map<String, ? extends LaneProvenance> e :
        getLaneProvenanceByProviderAndId(null).values()) {
      lps.addAll(e.values());
    }
    return lps;
  }
  ...
  public Collection<? extends LaneProvenance> getLaneProvenance(
    ...
    List<LaneProvenance> lps = new ArrayList<>();
    for (Map<String, ? extends LaneProvenance> e :
        getLaneProvenanceByProviderAndId(filters).values()) {
      lps.addAll(e.values());
    }
    return lps;
  }
  ...
  public Collection<? extends AnalysisProvenance> getAnalysisProvenance() {
    ...
    for (Collection<? extends AnalysisProvenance> e :
        getAnalysisProvenanceByProvider(null).values()) {
      aps.addAll(e);
    }
    return aps;
  }
  ...
  public Collection<? extends AnalysisProvenance> getAnalysisProvenance(
    ...
    List<AnalysisProvenance> aps = new ArrayList<>();
    for (Collection<? extends AnalysisProvenance> e :
        getAnalysisProvenanceByProvider(filters).values()) {
      aps.addAll(e);
    }
    return aps;
  }
  ...
  public Collection<? extends FileProvenance> getFileProvenance() {
    return getFileProvenance(
        getSampleProvenanceByProviderAndId(
            Collections.<FileProvenanceFilter, Set<String>>emptyMap()),
  ...
  public Collection<FileProvenance> getFileProvenance(
    ...
    for (Entry<String, Collection<AnalysisProvenance>> e :
        analysisProvenanceByProvider.entrySet()) {
      ...
      for (AnalysisProvenance ap : e.getValue()) {
        ...
        for (IusLimsKey ik : ap.getIusLimsKeys()) {
          ...
          if ((sp == null && lp == null)) {
            ...
            if (sp.getLastModified() == null
                || !sp.getLastModified().isEqual(limsKeyLastModified)) {
              status = Status.STALE;
            ...
            if (lp.getLastModified() == null
                || !lp.getLastModified().isEqual(limsKeyLastModified)) {
              status = Status.STALE;
              statusReasons.add(StatusReason.PROVENANCE_LAST_MODIFIED_MISMATCH);
            }
          ...
          b.setStatusReason(
              statusReasons.stream().map(StatusReason::toString).collect(Collectors.joining(",")));
  ...
  public Collection<FileProvenance> getFileProvenance(
      Map<FileProvenanceFilter, Set<String>> filters) {
    Collection<FileProvenance> fps =
        getFileProvenance(
            // get all sample and lane provenance and filter after joining with AP
            // if this is not done, there could be AP records that pass filters and do not have an
            // associated SP or LP
            // for example: AP record linked to SP(A) and SP(B), filtering on A would result in
            // FP(AP+SP(B))=ERROR
            // getSampleProvenanceByProviderAndId(filters),
            // getLaneProvenanceByProviderAndId(filters),
            getSampleProvenanceByProviderAndId(
                Collections.<FileProvenanceFilter, Set<String>>emptyMap()),
            getLaneProvenanceByProviderAndId(
                Collections.<FileProvenanceFilter, Set<String>>emptyMap()),
  ...
  @Override
  public Collection<FileProvenance> getFileProvenance(
      Map<FileProvenanceFilter, Set<String>> includeFilters,
      Map<FileProvenanceFilter, Set<String>> excludeFilters) {
  ...
  protected Collection<FileProvenance> applyFileProvenanceFilters(
      Collection<? extends FileProvenance> fps,
  ...
  protected Collection<FileProvenance> applyFileProvenanceFilters(
    ...
    List<Predicate<FileProvenance>> filterset =
        filters
            .entrySet()
            .stream()
            .<Predicate<FileProvenance>>map(
                entry -> f -> entry.getKey().checkFilter(f, entry.getValue()))
  ...
  public void close() throws Exception {
    final List<Exception> exceptions =
        Stream.<Collection<? extends AutoCloseable>>of(
                sampleProvenanceProviders.values(),
                laneProvenanceProviders.values(),
                analysisProvenanceProviders.values())
